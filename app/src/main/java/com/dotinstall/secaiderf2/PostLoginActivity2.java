...
public class PostLoginActivity2 extends AppCompatActivity implements View.OnClickListener {
    ...
    private HashMap<String, Integer> dateList = new HashMap<String, Integer>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.calendar_original_view);
...
//        Log.e("StoreOk", user.lineUserId);
            ...
            public void onResponse(Call<List<KoredakeTaiso>> call, Response<List<KoredakeTaiso>> response) {
                ...
                for (KoredakeTaiso k : koredake) {
                    try {
                        ...
                        if (dateList.containsKey(formattedDate)) {
                            // TODO: カレンダーに色つける、setCalendarでも行う

                            int excerciseDay = dateList.get(formattedDate);

                            //dayTextList.get(excerciseDay).getTextObject().setTextColor(Color.RED);
    ...
    private void initializeControl() {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 7; j++) {
                try{
                CustomImageView custom = (CustomImageView) findViewById(this.dayTextList.get(id).getTextViewId());
                ...
                */
                ...
                this.dayTextList.get(id).setTextObject(custom);
                //Log.e("custom","custom「" + custom + "」");
                id++;
    ...
    private void SetCalendar(int offset) {
        ...
        for(int i = 0 ; i < this.dayTextList.size(); i++) {
            ...
            tg.getTextObject().setInfoDate(tg.getDispString());
        ...
        for(int i = 0 ; i < this.dayTextList.size(); i++) {
            ...
            if(calendarInfo.calendarMatrix[row][col] != 0) {
                ...
                dateList.put(dateListElement, id);
                Log.e("dateList","dateListは「" + dateList + "」");
                ...
                try {
                    ...
                    tg.setNowDay(true);
                    tg.setSelected(true);
                    tg.getTextObject().setInfoDate(todayStr);
                ...
                if(this.nowYear == this.currentYear
                    ...
                    // 当日日付表示
                    //this.dayTextList.get(i).getTextObject().setTextColor(Color.GREEN);
                ...
                for (KoredakeTaiso k : koredake) {
                    try {
                        ...
                        if (dateList.containsKey(formattedDate)) {
                            ...
                            //dayTextList.get(excerciseDay).getTextObject().setTextColor(Color.BLACK);
                            Log.e("一致", formattedDate);
                        }
